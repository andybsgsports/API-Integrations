 ...
 * @NModuleScope Public
 *
 * BSG item form vendor-declutter (server-side, works in VIEW mode).
 *
 ...
 *
 * WHY A USER EVENT, NOT A CLIENT SCRIPT: a Client Script's pageInit does not
 * fire when a record is merely *viewed* (NetSuite renders view pages as static
 * HTML and never runs the client script). beforeLoad runs server-side on every
 * load -- including view -- and can hide the fields before the page renders, so
 * the declutter actually shows up when you open an item.
 *
 * Runs on VIEW and EDIT of existing records. On CREATE / COPY everything stays
 * visible so a manual item can still be filled in.
 *
 * Deploy as a User Event Script record with a deployment whose "Applies To" is
 * Inventory Part (and any other item types you use). See docs/CLIENT_SCRIPT_DEPLOY.md.
 */
define(['N/ui/serverWidget', 'N/log'], function (serverWidget, log) {
  ...
  var VENDORS = [
    ...
    },
    {
      ...
      fields: [
        ...
        'custitem_mtec_size_guide', 'custitem_mtec_instock_guaranteed'
      ]
    },
    {
      label: 'Under Armour',
    ...
    { label: 'Champro', marker: 'custitem_champro_part_id',
      fields: ['custitem_champro_part_id', 'custitem_champro_style',
        ...
        'custitem_champro_size_guide', 'custitem_champro_fabrics'] },
  ...
  function beforeLoad(context) {
    // Only declutter existing records being viewed/edited; on create/copy leave
    // everything visible so a manual item can still be filled in.
    var t = context.type;
    if (t !== context.UserEventType.VIEW && t !== context.UserEventType.EDIT) {
      return;
    }
    ...
    VENDORS.forEach(function (vendor) {
      var mv;
      try { mv = rec.getValue({ fieldId: vendor.marker }); }
      catch (e) { mv = null; }
      if (!isEmpty(mv)) {
        present++;
    ...
    log.debug({
      title: 'bsg_item_vendor_display_ue',
      details: 'type=' + t + '; vendors with data: ' + present +
